...
export class InstagramService {
    ...
    public async publishPhoto({
        file, caption, reason
    }: {file: Buffer<ArrayBufferLike>, caption: string, reason?: string}): Promise<void> {
        if(!vaildateCaption(caption)) return;
        ...
        try {
            ...
            logger.info(`사진 업로드 성공 (username: ${this.username}) ${reason ? `- reason: ${reason}` : ''}`);
        } catch (error) {
            logger.error(`사진 업로드 실패 (username: ${this.username}) ${reason ? `- reason: ${reason}` : ''}`);
            throw error;
        }
    }
}
