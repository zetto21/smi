...
client.on("messageCreate", async (message) => {
    if (message.author.id === "534214957110394881") {
        ...
        } else if (message.content.startsWith("!시간변경")) {
            const args = message.content.split(" ");
            if (args.length === 3) {
                const hour = parseInt(args[1]);
                const minute = parseInt(args[2]);
                if (!isNaN(hour) && !isNaN(minute) && hour >= 0 && hour < 24 && minute >= 0 && minute < 60) {
                    scheduleRandomJob(hour, minute);
                    await message.reply(`급식 전송 시간이 변경되었습니다: ${hour}시 ${minute}분`);
                } else {
                    await message.reply("⛔ 잘못된 시간 형식입니다. 올바른 형식: `!시간변경 [시] [분]` (예: `!시간변경 13 30`)");
                }
            } else {
                await message.reply("⛔ 잘못된 명령어 형식입니다. 올바른 형식: `!시간변경 [시] [분]` (예: `!시간변경 13 30`)");
            }
    ...
    } else if (message.content === "!재전송" || message.content.startsWith("!시간변경") || message.content === "!급식알림 끄기" || message.content === "!급식알림 켜기" ){
